/**
 * ═══════════════════════════════════════════════════════════════════════════════
 * 🎆 FIREWORKS ENGINE - birthday.js Extracted Module
 * ═══════════════════════════════════════════════════════════════════════════════
 * 
 * DESCRIPTION:
 * Complete fireworks display system with particle physics, audio effects, and
 * interactive controls. Triggered after the candle is blown out in birthday.js
 * 
 * KEY SYSTEMS:
 * - Particle Physics Engine: Gravity, air resistance, momentum, collision detection
 * - Multiple Shell Types: Burst, Floral, Palm, Ring, Willow, Crossette, Crackle
 * - Audio Manager: Dynamic sound effects with 5+ sound types
 * - Device Detection: Auto-adjusts quality for mobile/desktop/high-end devices
 * - Canvas Rendering: Hardware-accelerated with blend modes and trails
 * - User Controls: Shell type, size, quality, effects, fullscreen, etc.
 * 
 * DEPENDENCIES:
 * - Stage.js (https://s3-us-west-2.amazonaws.com/s.cdpn.io/329180/Stage%400.1.4.js)
 * - MyMath.js (https://s3-us-west-2.amazonaws.com/s.cdpn.io/329180/MyMath.js)
 * - FScreen.js (https://s3-us-west-2.amazonaws.com/s.cdpn.io/329180/fscreen%401.0.1.js)
 * 
 * EXPORTED FUNCTIONS:
 * - initFireworks(): Start the fireworks display (called by birthday.js)
 * - setLoadingStatus(status): Update loading message
 * 
 * USAGE:
 * <script src="fireworks.js"></script>
 * <script>
 *   // Called after candle is blown out
 ...
 * 
 * CUSTOMIZATION TIPS:
 * 1. GRAVITY: Adjust GRAVITY constant (line 26) for physics intensity
 *    - Higher = particles fall faster
 *    - Lower = particles float longer
 * 
 * 2. SHELL TYPES: Edit shell definitions in ShellTypes object (around line 700)
 *    - Each type defines particle count, colors, and behavior
 * 
 * 3. COLORS: Modify colorPalette array (around line 1500) for custom schemes
 * 
 * 4. AUDIO: Update soundManager.sources (around line 2330) for different effects
 *    - Change volume, playback rate, or audio file URLs
 * 
 * 5. PERFORMANCE: Adjust MAX_PARTICLES_PER_SHELL and quality presets for slower devices
 * 
 * ═══════════════════════════════════════════════════════════════════════════════
 */
...
const store = {
        ...
        load() {
                ...
                // Deprecated data format — size intentionally not restored.
        },
...
const _fwRoot = document.getElementById('fireworksSection') || document;
Object.keys(appNodes).forEach(key => {
        appNodes[key] = _fwRoot.querySelector(appNodes[key]);
});
...
function init() {
        ...
        _fwRoot.querySelector('.loading-init').remove();
...
function seqTwoRandom() {
        ...
        return 500 + Math.random() * 400 + extraDelay;
}
...
function startSequence() {
        ...
        // Always launch at least 2 fireworks — seqRandomShell (single) removed
...
function setLoadingStatus(status) {
        (_fwRoot || document).querySelector('.loading-init__status').textContent = status;
}

// Called by birthday.js after candle is blown out
function initFireworks() {
        setTimeout(() => {
                ...
                .then(
                        () => {
                                init();
                                // Enable sound immediately (always on)
                                toggleSound(true);
                                soundManager.resumeAll();
                        },
                        reason => {
                                init();
                                // Still enable sound even if preload fails
                                toggleSound(true);
